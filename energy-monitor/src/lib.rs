...
async fn try_scheduled(env: Env) -> Result {
    let (p1_measurement, battery_measurement) = {
        let p1_client = Client(env.service("p1Service")?);
        let battery_meter_client = Client(env.service("batteryMeterService")?);
        futures::try_join!(
